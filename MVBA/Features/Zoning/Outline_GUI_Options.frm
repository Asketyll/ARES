...
Private Sub TextBox_Levels_Exit(ByVal Cancel As MSForms.ReturnBoolean)
    On Error GoTo ErrorHandler
    If TextBox_Levels.Value <> ARESConfig.ARES_OUTLINE_LEVEL.Value Then
        ARESConfig.ARES_OUTLINE_LEVEL.Value = TextBox_Levels.Value
    End If
...
Private Sub TextBox_Distance_Exit(ByVal Cancel As MSForms.ReturnBoolean)
    ...
    If Val(normalized) <= 0 Then
        Cancel = True
        MsgBox GetTranslation("OutlineGUIOptionsDistanceError"), vbOKOnly
        TextBox_Distance.Value = ARESConfig.ARES_OUTLINE_DISTANCE.Value
        Exit Sub
    End If
    If normalized <> ARESConfig.ARES_OUTLINE_DISTANCE.Value Then
        ARESConfig.ARES_OUTLINE_DISTANCE.Value = normalized
...
Private Sub TextBox_OutputLevel_Exit(ByVal Cancel As MSForms.ReturnBoolean)
    On Error GoTo ErrorHandler
    If TextBox_OutputLevel.Value <> ARESConfig.ARES_OUTLINE_OUTPUT_LEVEL.Value Then
        ARESConfig.ARES_OUTLINE_OUTPUT_LEVEL.Value = TextBox_OutputLevel.Value
    End If
...
Private Sub Weight_SpinButton_Change()
    On Error GoTo ErrorHandler
    If mbLocked Then
    ElseIf Weight_SpinButton.Value <> CLng(ARESConfig.ARES_OUTLINE_OUTPUT_WEIGHT.Value) Then
        Locked
...
ErrorHandler:
    ErrorHandler.HandleError Err.Description, Err.Number, Err.Source, "Outline_GUI_Options.Locked"
    Locked = False
End Function
...
ErrorHandler:
    ErrorHandler.HandleError Err.Description, Err.Number, Err.Source, "Outline_GUI_Options.CheckControlForLock"
End Sub
...
ErrorHandler:
    ErrorHandler.HandleError Err.Description, Err.Number, Err.Source, "Outline_GUI_Options.SeeActiveTextBox"
End Sub
