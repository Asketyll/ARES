...
- **Zone Export** (`ExportLengthInRegion.bas`; `ExportLength`): for each element, `Length.GetPartialLengthInsideZones` (length inside zone polygons), aggregated by Style/Level/Color, written to a new `.xlsx`. An optional `ARES_Zone_Export_Level` filter (`|`-list; empty = all levels) restricts which measured elements are counted (non-existent names are ignored, reported on the status bar). An optional **per-zone breakdown** (checkbox `ARES_Zone_Export_Per_Zone`) additionally splits each group by zone (long format `Zone | group | length`, one row per zone×group, zones never merged), each zone labelled by a custom property read from the zone element (`ARES_Zone_Export_Zone_Property`; empty/absent → `Zone <n>`). The Excel COM lifecycle is carefully managed (never quits the user's own session).
