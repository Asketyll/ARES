...
| | `PropertyCalculation/PropertyCalculation.bas` (+ `PropertyCalculation_GUI_Options`) | value-calculation engine for custom properties (AWAKE phase 2): per-property calc rules `Prop[name][&cond]=Source` (`ARES_Calc_Rules`); sources `CellText`/`Value`/`Coord`/`Id`; first-match wins; options via `EditPropertyCalculationOptions` |
...
**`ChangeHandler.ProcessElement(oElement, Depth=0)`** — `MAX_DEPTH = 20` recursion cap. At Depth 0 (before the graphic-group=0 skip): if `ARES_Auto_Properties` → `PropertyTagging.ApplyPropertyRules`; then if enabled → `PropertyCalculation.ProcessElement` (bearing pass + trigger-cell pass). Deletion needs no dedicated calculation hook: deleting a trigger cell already re-queues its former group members, whose bearing pass re-resolves each carried calc property against the current group (surviving cell's text, or `""`).
...
| `ARES_Property_Calc` / `ARES_Calc_Rules` | False / (empty) | PropertyCalculation switch + calc rules `Prop[name][&cond]*=Source ; …` (Source = `CellText[pattern]`/`Value[text]`/`Coord[n]`/`Id`; first-match wins) |
