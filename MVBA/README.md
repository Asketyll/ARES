...
**`ChangeHandler.ProcessElement(oElement, Depth=0)`** — `MAX_DEPTH = 20` recursion cap. At Depth 0 (before the graphic-group=0 skip): if `ARES_Auto_Properties` → `PropertyTagging.ApplyPropertyRules` (self-attach + `@` group push + `@` pull); then if enabled → `PropertyCalculation.ProcessElement` (bearing pass + trigger-cell pass). Deletion needs no dedicated calculation hook: deleting a trigger cell already re-queues its former group members, whose bearing pass re-resolves each carried calc property against the current group (surviving cell's text, or `""`).
...
- **Property Calculation** (`PropertyCalculation.bas`, ex-Property Propagation; options via `EditPropertyCalculationOptions` → `PropertyCalculation_GUI_Options`; master switch `ARES_Property_Calc`, OFF by default): **a value-calculation engine only** — it never attaches/detaches (any direct attach/detach in this module is a review BLOCKER); it writes only where a property is **already attached** (the frontier `IsItemAttachedToElement`). **AWAKE (phase 2):** each custom property's value comes from a **calc rule** in `ARES_Calc_Rules`, grammar `Prop[name] [&condition]* = Source`, built on the shared `RuleGrammar` conditions (`Lvl/Cell/Type`, `&`, `!`, `*`/`?`). `Prop[name]` is the **target** (one name, **no wildcard**, syntactic-only — the frontier is the runtime guard). Six **sources** (keyword + optional `[arg]`, arity-checked, unknown rejected fail-closed): `CellText[pattern]` (the full text via `StringsInEl.GetConcatenatedText` of a group cell whose name matches `pattern`, **including the bearing element itself** — an ungrouped matching cell is a group of one), `CellCoord[pattern]`/`CellId[pattern]` (the coordinates/`DLongToString` id of that SAME matching cell, not the bearing element's own — use this to get the TAGGING cell's position/id on a member it pushed properties to; `CellId` is stable, never pushed), `Value[text]` (fixed literal), `Coord`/`Coord[n]` (the bearing element's OWN `"X;Y"` via a verified anchor cascade `Origin → CenterPoint → Centroid → Range-centre`, master units, `Round`+`CStr`, default decimals = `ARES_Round`), `Id` (the bearing element's OWN `DLongToString`). Every `Cell*` source's `pattern` may be several `ARES_VAR_DELIMITER` (`\|`) - separated alternatives (`MatchesAnyPattern`, same grammar as a tag/calc CONDITION's `Cell[name|name]`). **Several rules for one property: the FIRST that matches wins** (order = priority; specific rules first). One bracket-depth-aware parser (`ParseCalcRule`) backs the runtime cache, `ValidateAndNormalizeCalcRule` (targeted reason OR the compact canonical the editor stores) and `CalcRuleHasNoEffect` (dead-condition detector via `RuleGrammar.ConditionsHaveContradiction`, the `Prop` target ignored). **Engine:** two Depth-0 passes — a **bearing pass** (fill/recompute each carried property from its first-matching rule; also the delete-reconcile — a re-queued member re-resolves to the surviving cell's text/coord, or `""`), and a **trigger-cell pass** (a cell matching a `CellText[pattern]` or `CellCoord[pattern]` pushes its text/coordinates to the OTHER members MicroStation did not re-queue, first-match guarded; `CellId` never needs a push — an ID cannot change). Every write routes through `ApplyValueToSibling` (frontier + compare-before-write + non-empty→empty **transition guard** + `ARES_Calc_Detach_Empty` delegated detach — all loop-safety BLOCKERs) and **addresses the target property's OWN item** (`ItemName:=P` — resolving the first-attached item instead misdirected calc writes on elements carrying ≥ 2 properties). One-shot statuses `CalculationNoTarget` / `CalculationValueRejected` / `CalculationMultipleTriggers`. The options GUI (`EditPropertyCalculationOptions`) edits the calc rules one at a time through `ComboBox_CalcRules` + a coloured preview `Frame_CalcPreview`, reusing the shared `RuleEditorUX` (same mechanics/sentinel/close-flush as the tag rule editor) driven by `ValidateAndNormalizeCalcRule` / `CalcRuleHasNoEffect`; the preview bolds the source keywords `CellText`/`CellCoord`/`CellId`/`Value`/`Coord`/`Id` alongside `Prop`/`Lvl`/`Cell`/`Type`; a malformed rule is refused status-only (`CalcRuleInvalid`, never logged).
...
- **StringsInEl.bas** — `GetSetTextsInEl(el, txt, Triggers, Color)`: get/set text in Text/TextNode/Cell. GET on a single `TextElement` returns a **per-character** array (trap), and on a cell returns only the LAST text-bearing sub-element — it does NOT aggregate a cell's text. For that use `GetConcatenatedText(El, Separator)`: read-only recursive DFS returning ALL text in an element (whole strings, text-node lines, nested cells), trimmed, empties dropped, space-joined. TextNode writes per sub-element (the `TextLine` write property is buggy).
