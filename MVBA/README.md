...
| | `PropertyPropagation/PropertyPropagation.bas` (+ `PropertyPropagation_GUI_Options`) | propagate a trigger "label cell"'s full text as a custom-property value onto its graphic-group siblings; options via `EditPropagationOptions` |
| **Command/** | `Command.bas` | the key-in surface; each key-in inits config/lang, delegates |
| **Components/** | `Geometry`, `Length`, `Link`, `StringsInEl`, `GetElements`, `CustomPropertyHandler`, `MicroStationDefinition`, `MSGraphicalInteraction`, `CellRedreaw`, `FileDialogs`, `FormUXHelper`, `FormPlacement` | shared helpers (see [Shared components](#shared-components-key-apis)) |
...
**`ChangeHandler.ProcessElement(oElement, Depth=0)`** — `MAX_DEPTH = 20` recursion cap. At Depth 0 (before the graphic-group=0 skip): if `ARES_Auto_Properties` → `PropertyTagging.ApplyPropertyRules`; then if enabled → `PropertyPropagation.ProcessElement` (detach-consume + propagate). The Delete branch also records propagation detach intent (`NoteDeletedTriggerCell`, read-only — never writes inside the change event).
...
- **Property Tagging** (`PropertyTagging.bas`; automatic; options via `EditPropertyTaggingOptions` → `PropertyTagging_GUI_Options`; master switch `ARES_Auto_Properties`): **the sole attach/detach engine for custom properties**. On element add/modify, attaches per `ARES_Property_Rules` — syntax `level[:type]=prop|prop ; @CellName=prop|prop ; …` (rules separated by `;`, props within a rule by `|`). **Level rules** attach to the matching element itself (level name + optional element type; non-graphical / level-less elements are silently skipped). **Cell-group rules** (`@` selector) fan out from a trigger CELL whose `Name` matches, attaching to every other member of its graphic group — evaluated only when the processed element is the named cell, never from a member; this pass runs before the level guards (a cell header has no Level). Attaches are `HasItems`-guarded idempotent; the new public `DetachRuleProperty(El, P)` is the only sanctioned detach service (used by Property Propagation's opt-in detach-on-empty). Rules parsed + cached (`RefreshRules`); ungrouped level-rule matches are also queued; everything applies at `Depth = 0` only. The value engine reads the `@` rules through the read-only `GetCellGroupProperties(cellName)` accessor (single source for its triggers/targets). The options GUI edits the rules **one at a time** through an editable dropdown (pick → edit → commit = replace; free-type = add; empty = delete), and **every commit — dropdown or raw-paste — is guarded by `ValidateRuleSyntax`** (read-only, built on the parser's own grammar constants): a malformed rule (e.g. `|` used between rules instead of `;`) is refused with the one-shot `PropertyRuleInvalid` status instead of being silently swallowed.
- **Property Propagation** (`PropertyPropagation.bas`; automatic; options via `EditPropagationOptions` → `PropertyPropagation_GUI_Options`; master switch `ARES_Property_Propagation`, OFF by default): **a value-calculation engine only** — it never attaches/detaches (any direct attach/detach in this module is a review BLOCKER). Its **triggers and targets come from the `@cell` rules** (epic 12 — the single source, read via the read-only `PropertyTagging.GetCellGroupProperties`; the engine owns no cell-name / target-property config). A `@X=P` rule makes cell `X` a trigger and `P` its target; when `X` (in a real graphic group) is added or modified, the cell's **full concatenated text** (`StringsInEl.GetConcatenatedText`) is written as the value of **each** target property (per-rule; `@X=P1|P2` fills both, rules with the same name aggregate) onto every other group member **that already carries it** (frontier check `IsItemAttachedToElement`; attach is the rules' job — typically the same `@X` rule in the same Depth-0 pass, attach-then-value order). No member carries any target → one-shot `PropagationNoTarget` hint (a residual "Tagging off / DGNLib missing" guard). **Loop-safety (review BLOCKER)**: every set/clear is compare-before-write, and the empty branch is **transition-guarded** (fires only on a real non-empty→empty transition; without it, detach-on-empty oscillates against a re-attaching rule). Emptied values (text cleared, or trigger cell deleted with no surviving trigger — deferred + reconciled per target; a survivor targeting that property re-propagates its text instead): with `ARES_Propagation_Detach_Empty` OFF (default) the value is cleared and the property stays attached; ON → delegates a detach to `PropertyTagging.DetachRuleProperty` (stays detached until a new trigger cell is added; a still-governing rule re-attaches empty — rules win, converges). Several trigger cells whose targets **share a property** → one-shot `PropagationMultipleTriggers` warning (status-only, scoped per shared target; different-property cells are silent); last-modified wins. Shared cells and non-active models are out of scope.
...
| `ARES_Auto_Properties` / `ARES_Property_Rules` | True / (empty) | PropertyTagging switch + rules `level[:type]=prop\|prop ; @CellName=prop ; …` (`;` between rules) |
