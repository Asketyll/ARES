...
| **Configuration/** | `ARESConfigClass.cls` | registry of ~36 MS config vars (global `ARESConfig`); versioned import/export |
...
- **Property Tagging** (`PropertyTagging.bas`; automatic; options via `EditPropertyTaggingOptions` → `PropertyTagging_GUI_Options`; master switch `ARES_Auto_Properties`): on element add/modify, auto-attaches custom properties per `ARES_Property_Rules` (`level[:type]=prop|prop ; …`). Matches by level name (+ optional element type); properties from `ARES_Custom_Property_List`, attached via `CustomPropertyHandler.AttachItemToElement` (attach-only, idempotent). Rules are parsed + cached (`RefreshRules` reloads; the options GUI calls it after a rule edit so changes apply live, no restart). It also queues **ungrouped** matching elements, extending capture beyond the graphic-group AutoLengths workflow; attaches at `Depth = 0` only. Rule matching is level-based, so non-graphical / level-less elements (e.g. cell headers) are silently skipped.
- **Property Propagation** (`PropertyPropagation.bas`; automatic; options via `EditPropagationOptions` → `PropertyPropagation_GUI_Options`; master switch `ARES_Property_Propagation`, OFF by default): when a trigger "label cell" (a `CellElement` whose `Name` is in the `ARES_Propagation_Cells` `|`-list, in a real graphic group) is added or modified, writes the cell's **full concatenated text** (`StringsInEl.GetConcatenatedText`) as the value of the user-chosen custom property (`ARES_Propagation_Property`) onto every other group member (`Link.GetLink`), attach+set via `CustomPropertyHandler`. **Loop-safety invariant (review BLOCKER): every attach/set/detach is compare-before-write** — item-type writes persist immediately and re-fire Modify events; only the compare guard makes the cascade settle. Deletion detach is **deferred + reconciled**: the Delete branch records intent only; on idle a former sibling is detached only when no trigger cell remains in its group, else re-propagated to the surviving trigger cell's value (first in scan order). Several trigger cells in one group → translated one-shot warning (`PropagationMultipleTriggers`, status-only — a config choice, not a fault); last-modified wins; convergence is only guaranteed with a single trigger per group. Shared cells and non-active models are out of scope.

## Configuration system

- `ARESConfig` (global) holds ~36 vars; each `ARES_MS_VAR_Class` write-throughs to MicroStation on `.Value` assignment. First run seeds defaults (`Config.GetVar` == `ARES_NAVD` → write default).
