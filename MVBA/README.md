...
| | `PropertyCalculation/PropertyCalculation.bas` (+ `PropertyCalculation_GUI_Options`) | calculate a custom-property value from a trigger "label cell"'s full text onto its graphic-group siblings; options via `EditPropertyCalculationOptions` |
...
**`ChangeHandler.ProcessElement(oElement, Depth=0)`** — `MAX_DEPTH = 20` recursion cap. At Depth 0 (before the graphic-group=0 skip): if `ARES_Auto_Properties` → `PropertyTagging.ApplyPropertyRules`; then if enabled → `PropertyCalculation.ProcessElement` (detach-consume + calculate). The Delete branch also records calculation detach intent (`NoteDeletedTriggerCell`, read-only — never writes inside the change event).
...
- **Property Calculation** (`PropertyCalculation.bas`, ex-Property Propagation; options via `EditPropertyCalculationOptions` → `PropertyCalculation_GUI_Options`; master switch `ARES_Property_Calc`, OFF by default): **a value-calculation engine only** — it never attaches/detaches (any direct attach/detach in this module is a review BLOCKER). **DORMANT in phase 1**: the `@` rules no longer feed the engine (`IsTriggerCell` returns False, empty trigger derivation); the write machinery is retained intact as phase-2 scaffolding — `ApplyValueToSibling` (frontier check `IsItemAttachedToElement`, compare-before-write, **transition-guarded** empty branch), the per-target deferred reconcile, `ARES_Calc_Detach_Empty` (ON → detach delegated to `PropertyTagging.DetachRuleProperty`) and the one-shot statuses (`CalculationNoTarget` / `CalculationValueRejected` / `CalculationMultipleTriggers`). **Phase 2 (planned): per-property calculation rules in its GUI — a computed value (e.g. the group label-cell's full text via `StringsInEl.GetConcatenatedText`) or a fixed value, consulted when a property is attached.** The loop-safety invariants stay review BLOCKERs when the engine wakes.
