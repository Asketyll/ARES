...
Sub InitializeTranslations()
    ...
    moTranslations.Add "EN_CalculationUnknownGeoSystem", "ARES: Property calculation - the requested output system name in a Coord/CellCoord rule is not recognised by MicroStation's Geographic Coordinate System library"
    moTranslations.Add "EN_CalcRuleInvalid", "ARES: Calc rule invalid, not saved (expected Prop[name][&...]=Source; Source = CellText|CellCoord|CellId|CellLvl|CellColor|CellStyle|CellWeight|LvlColor|LvlStyle|LvlWeight|GroupProp[pattern] / Value[text] / Coord|Length|GroupLength[n] / Id|Lvl|Color|Style|Weight|GroupColor)"
    moTranslations.Add "EN_CalculationGUIOptionsCalcRules_Tip", "Calc rules: Prop[name] [&Lvl/Cell/Type[..]] = CellText|CellCoord|CellId|CellLvl|CellColor|CellStyle|CellWeight|LvlColor|LvlStyle|LvlWeight[pattern] | GroupProp[PropertyName] | Value[text] | Coord|Length|GroupLength[n] | Id|Lvl|Color|Style|Weight|GroupColor. pattern may use '|' for several name alternatives, e.g. ASUF*|SP0*. First matching rule per property wins (specific rules first). Example: Prop[Repere]&Cell[ETIREF]=Value[REF] ; Prop[Repere]=CellText[ETI*] ; Prop[XY]=Coord ; Prop[Coordonnee]=CellCoord[ASUF*|SP0*] ; Prop[Longueur]=GroupLength[1] ; Prop[Nature]=GroupProp[Nature]"
    ...
    moTranslations.Add "FR_CalculationUnknownGeoSystem", "ARES : Calcul de propriété - le nom de système de sortie demandé dans une règle Coord/CellCoord n'est pas reconnu par la bibliothèque de systèmes de coordonnées géographiques de MicroStation"
    moTranslations.Add "FR_CalcRuleInvalid", "ARES : Règle de calcul invalide, non enregistrée (attendu Prop[nom][&...]=Source ; Source = CellText|CellCoord|CellId|CellLvl|CellColor|CellStyle|CellWeight|LvlColor|LvlStyle|LvlWeight|GroupProp[motif] / Value[texte] / Coord|Length|GroupLength[n] / Id|Lvl|Color|Style|Weight|GroupColor)"
    moTranslations.Add "FR_CalculationGUIOptionsCalcRules_Tip", "Règles de calcul : Prop[nom] [&Lvl/Cell/Type[..]] = CellText|CellCoord|CellId|CellLvl|CellColor|CellStyle|CellWeight|LvlColor|LvlStyle|LvlWeight[motif] | GroupProp[NomPropriete] | Value[texte] | Coord|Length|GroupLength[n] | Id|Lvl|Color|Style|Weight|GroupColor. Le motif peut utiliser '|' pour plusieurs alternatives de nom, ex. ASUF*|SP0*. La première règle qui correspond par propriété gagne (règles spécifiques d'abord). Exemple : Prop[Repere]&Cell[ETIREF]=Value[REF] ; Prop[Repere]=CellText[ETI*] ; Prop[XY]=Coord ; Prop[Coordonnee]=CellCoord[ASUF*|SP0*] ; Prop[Longueur]=GroupLength[1] ; Prop[Nature]=GroupProp[Nature]"
    ' --- Custom-property DGNLib round trip (OpenPropertyLibrary key-in) ---
