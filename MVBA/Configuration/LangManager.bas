...
Sub InitializeTranslations()
    ...
    moTranslations.Add "EN_LengthConduitFallbackToLongestSide", "Element: {0} has too few sub-elements to isolate 2 conduit end caps; falling back to the longest single sub-element for its length."
    ...
    moTranslations.Add "EN_ZoneExportHeaderStyle", "Line Style"
    moTranslations.Add "EN_ZoneExportHeaderLevel", "Level"
    moTranslations.Add "EN_ZoneExportHeaderColor", "Color"
    moTranslations.Add "EN_ZoneExportHeaderLength", "Total Length ({0})"
    moTranslations.Add "EN_ZoneExportHeaderZone", "Zone"
    moTranslations.Add "EN_ZoneExportHeaderID", "ID"
    moTranslations.Add "EN_ZoneExportMasterUnitFallback", "master units"
    ...
    moTranslations.Add "EN_GroupSplitGuardIntro", "This cut left {0} cable pieces in the same graphic group ({1}): the texts that read their values off the group no longer know which piece to read."
    moTranslations.Add "EN_GroupSplitGuardYesSeparate", "Yes: separate - each piece gets its own group, and every other element joins the piece it lies nearest to."
    moTranslations.Add "EN_GroupSplitGuardCancelUndo", "Cancel: undo the cut."
    moTranslations.Add "EN_GroupSplitGuardNoKeep", "No: keep everything as it is."
    moTranslations.Add "EN_GroupSplitGuardUndoUnavailable", "Undoing the cut is no longer offered: something else changed since."
    moTranslations.Add "EN_GroupSplitGuardSeparated", "ARES: group separated into {0}; {1} element(s) moved to a new piece, {2} locked element(s) left in place"
    ...
    moTranslations.Add "EN_CalculationGUIOptionsMain_LabelTip", "When on, properties get their values from the calculation rules below (Prop[name]=Source): a group label cell's text (GroupCellText) or coordinates (GroupCellCoord) or ID (GroupCellId), a fixed value (Value), the element's own coordinates (Coord) or its own ID (Id). Values are only written where the property is already attached - attaching is Property Tagging's job."
    ...
    moTranslations.Add "EN_CalculationMultiplePropCandidates", "ARES: Property calculation - linked elements in this group carry DIFFERENT values for the source property; the first one found sets it"
    ...
    moTranslations.Add "EN_CalculationNoGeoReference", "ARES: Property calculation - no georeferencing configured for this model; run MicroStation's GEOCOORDINATE SELECT LIBRARY key-in to configure one, then re-edit the element"
    moTranslations.Add "EN_CalculationUnknownGeoSystem", "ARES: Property calculation - the requested output system name in a Coord/GroupCellCoord rule is not recognised by MicroStation's Geographic Coordinate System library"
    moTranslations.Add "EN_CalcRuleInvalid", "ARES: Calc rule invalid, not saved (expected Prop[name][&...]=Source; Source = GroupCellText|GroupCellCoord|GroupCellId|GroupCellLvl|GroupCellColor|GroupCellStyle|GroupCellWeight|GroupLvlColor|GroupLvlStyle|GroupLvlWeight[pattern] / GroupProp[PropertyName] / Value[text] / Coord|Length|GroupLength[n] / Id|Lvl|Color|Style|Weight|GroupColor)"
    moTranslations.Add "EN_CalculationGUIOptionsCalcRules_Tip", "Calc rules: Prop[name] [&Lvl/Cell/Type[..]] = GroupCellText|GroupCellCoord|GroupCellId|GroupCellLvl|GroupCellColor|GroupCellStyle|GroupCellWeight|GroupLvlColor|GroupLvlStyle|GroupLvlWeight[pattern] | GroupProp[PropertyName] | Value[text] | Coord|Length|GroupLength[n] | Id|Lvl|Color|Style|Weight|GroupColor. pattern may use '|' for several name alternatives, e.g. ASUF*|SP0*. First matching rule per property wins (specific rules first). Example: Prop[Repere]&Cell[ETIREF]=Value[REF] ; Prop[Repere]=GroupCellText[ETI*] ; Prop[XY]=Coord ; Prop[Coordonnee]=GroupCellCoord[ASUF*|SP0*] ; Prop[Longueur]=GroupLength[1] ; Prop[Nature]=GroupProp[Nature]"
    ...
    moTranslations.Add "EN_RecalculateNoSelection", "ARES: Recalculate - select the element(s) to recalculate first"
    moTranslations.Add "EN_RecalculateComplete", "ARES: Recalculate complete — {0} element(s)"
    ...
    ' --- Cable Report (per-cable trenching length by soil type) ---
    moTranslations.Add "EN_CableReportNoActiveModel", "ARES: Cable report — no active model reference"
    moTranslations.Add "EN_CableReportLevelNotConfigured", "ARES: Cable report — cable level not configured"
    moTranslations.Add "EN_CableReportLevelNotFound", "ARES: Cable report — cable level not found: {0}"
    moTranslations.Add "EN_CableReportLevelsIgnored", "ARES: Cable report — cable level(s) ignored (not found): {0}"
    moTranslations.Add "EN_CableReportCancelled", "ARES: Cable report — cancelled"
    moTranslations.Add "EN_CableReportNoCables", "ARES: Cable report — no cables found on the configured level"
    moTranslations.Add "EN_CableReportNoZones", "ARES: Cable report — no trenching zones found; soil-type columns will be empty"
    moTranslations.Add "EN_CableReportPropertyInvalid", "ARES: Cable report — {0} property invalid, ignored: {1}"
    moTranslations.Add "EN_CableReportComplete", "ARES: Cable report complete — {0} cable(s), {1} soil-type column(s), {2} incomplete, {3} shared trench zone(s)"
    moTranslations.Add "EN_CableReportSheetName", "Cable Report"
    moTranslations.Add "EN_CableReportHeaderRepere", "Repere"
    moTranslations.Add "EN_CableReportHeaderNature", "Nature"
    moTranslations.Add "EN_CableReportHeaderLongueur", "Length"
    moTranslations.Add "EN_CableReportHeaderSharedWith", "Shared with"
    moTranslations.Add "EN_CableReportFooterCableTotal", "Cable totals"
    moTranslations.Add "EN_CableReportFooterTrenchTotal", "Trench to dig (deduplicated)"
    ' Cable Report options GUI
    moTranslations.Add "EN_CableReportGUIOptionsCaption", "Edit cable report options:"
    moTranslations.Add "EN_CableReportGUIOptionsEditCableLevel_CommandCaption", "Edit cable level"
    moTranslations.Add "EN_CableReportGUIOptionsEditCableLevel_CommandTip", "Level holding the HTAS cable Line elements to report on."
    moTranslations.Add "EN_CableReportGUIOptionsEditZoneLevel_CommandCaption", "Edit zone level"
    moTranslations.Add "EN_CableReportGUIOptionsEditZoneLevel_CommandTip", "Level holding the trenching zones (RunOutline output). Empty falls back to the Outline output level."
    moTranslations.Add "EN_CableReportGUIOptionsRepereProperty_LabelCaption", "Repere property:"
    moTranslations.Add "EN_CableReportGUIOptionsRepereProperty_LabelTip", "Custom property read off each end cell for the cable's Repere. Must be an ItemType of the ARES DGNLib."
    moTranslations.Add "EN_CableReportGUIOptionsNatureProperty_LabelCaption", "Nature property:"
    moTranslations.Add "EN_CableReportGUIOptionsNatureProperty_LabelTip", "Custom property read off the cable's linked text for its nature/type. Must be an ItemType of the ARES DGNLib."
    moTranslations.Add "EN_CableReportGUIOptionsLongueurProperty_LabelCaption", "Longueur property:"
    moTranslations.Add "EN_CableReportGUIOptionsLongueurProperty_LabelTip", "Custom property read off the cable's linked text for its total length. Must be an ItemType of the ARES DGNLib."
    moTranslations.Add "EN_CableReportGUIOptionsZoneProperty_LabelCaption", "Soil-type property:"
    moTranslations.Add "EN_CableReportGUIOptionsZoneProperty_LabelTip", "Custom property read off each trenching zone; pivoted into one column per distinct value. Must be an ItemType of the ARES DGNLib."
    moTranslations.Add "EN_CableReportGUIOptionsSearchRadius_LabelCaption", "Search radius:"
    moTranslations.Add "EN_CableReportGUIOptionsSearchRadius_LabelTip", "Search radius (master units) for the nearest end-cell lookup around each cable endpoint. Must be a positive number."
    moTranslations.Add "EN_CableReportGUIOptionsSearchRadiusError", "Search radius must be a positive number."
    moTranslations.Add "EN_CableReportGUIOptionsRound_LabelCaption", "Decimal places:"
    moTranslations.Add "EN_CableReportGUIOptionsRound_LabelTip", "Decimal places for the pivoted soil-type length sums (0-10)."
    ...
    moTranslations.Add "FR_LengthConduitFallbackToLongestSide", "L'élément: {0} a trop peu de sous-éléments pour isoler 2 capuchons de fourreau ; retour au plus long sous-élément unique pour sa longueur."
    ...
    moTranslations.Add "FR_ZoneExportHeaderStyle", "Style de trait"
    moTranslations.Add "FR_ZoneExportHeaderLevel", "Niveau"
    moTranslations.Add "FR_ZoneExportHeaderColor", "Couleur"
    moTranslations.Add "FR_ZoneExportHeaderLength", "Longueur totale ({0})"
    moTranslations.Add "FR_ZoneExportHeaderZone", "Zone"
    moTranslations.Add "FR_ZoneExportHeaderID", "ID"
    moTranslations.Add "FR_ZoneExportMasterUnitFallback", "unités de dessin"
    ' --- Property Tagging (custom-property) options GUI ---
    ...
    moTranslations.Add "FR_GroupSplitGuardIntro", "Cette coupure a laissé {0} morceaux de câble dans le même groupe graphique ({1}) : les textes qui lisent leurs valeurs sur le groupe ne savent plus quel morceau lire."
    moTranslations.Add "FR_GroupSplitGuardYesSeparate", "Oui : séparer - chaque morceau reçoit son propre groupe, et chaque autre élément rejoint le morceau le plus proche."
    moTranslations.Add "FR_GroupSplitGuardCancelUndo", "Annuler : annuler la coupure."
    moTranslations.Add "FR_GroupSplitGuardNoKeep", "Non : tout garder tel quel."
    moTranslations.Add "FR_GroupSplitGuardUndoUnavailable", "L'annulation de la coupure n'est plus proposée : une autre modification a eu lieu depuis."
    moTranslations.Add "FR_GroupSplitGuardSeparated", "ARES : groupe séparé en {0} ; {1} élément(s) rattaché(s) à un nouveau morceau, {2} élément(s) verrouillé(s) laissé(s) en place"
    ...
    moTranslations.Add "FR_CalculationMultiplePropCandidates", "ARES : Calcul de propriété - des éléments liés de ce groupe portent des valeurs DIFFÉRENTES pour la propriété source ; le premier trouvé impose la sienne"
    ...
    moTranslations.Add "FR_CalculationNoGeoReference", "ARES : Calcul de propriété - aucun géoréférencement configuré pour ce modèle ; lancez la commande MicroStation GEOCOORDINATE SELECT LIBRARY pour en configurer un, puis modifiez de nouveau l'élément"
    moTranslations.Add "FR_CalculationUnknownGeoSystem", "ARES : Calcul de propriété - le nom de système de sortie demandé dans une règle Coord/GroupCellCoord n'est pas reconnu par la bibliothèque de systèmes de coordonnées géographiques de MicroStation"
    moTranslations.Add "FR_CalcRuleInvalid", "ARES : Règle de calcul invalide, non enregistrée (attendu Prop[nom][&...]=Source ; Source = GroupCellText|GroupCellCoord|GroupCellId|GroupCellLvl|GroupCellColor|GroupCellStyle|GroupCellWeight|GroupLvlColor|GroupLvlStyle|GroupLvlWeight[motif] / GroupProp[nomPropriété] / Value[texte] / Coord|Length|GroupLength[n] / Id|Lvl|Color|Style|Weight|GroupColor)"
    moTranslations.Add "FR_CalculationGUIOptionsCalcRules_Tip", "Règles de calcul : Prop[nom] [&Lvl/Cell/Type[..]] = GroupCellText|GroupCellCoord|GroupCellId|GroupCellLvl|GroupCellColor|GroupCellStyle|GroupCellWeight|GroupLvlColor|GroupLvlStyle|GroupLvlWeight[motif] | GroupProp[NomPropriete] | Value[texte] | Coord|Length|GroupLength[n] | Id|Lvl|Color|Style|Weight|GroupColor. Le motif peut utiliser '|' pour plusieurs alternatives de nom, ex. ASUF*|SP0*. La première règle qui correspond par propriété gagne (règles spécifiques d'abord). Exemple : Prop[Repere]&Cell[ETIREF]=Value[REF] ; Prop[Repere]=GroupCellText[ETI*] ; Prop[XY]=Coord ; Prop[Coordonnee]=GroupCellCoord[ASUF*|SP0*] ; Prop[Longueur]=GroupLength[1] ; Prop[Nature]=GroupProp[Nature]"
    ' --- Custom-property DGNLib round trip (OpenPropertyLibrary key-in) ---
    ...
    moTranslations.Add "FR_RecalculateNoSelection", "ARES : Recalcul - sélectionnez d'abord le ou les éléments à recalculer"
    moTranslations.Add "FR_RecalculateComplete", "ARES : Recalcul terminé — {0} élément(s)"
    ...
    ' --- Rapport de câbles (longueur de tranchée par nature de sol) ---
    moTranslations.Add "FR_CableReportNoActiveModel", "ARES : Rapport de câbles — aucun modèle actif"
    moTranslations.Add "FR_CableReportLevelNotConfigured", "ARES : Rapport de câbles — niveau des câbles non configuré"
    moTranslations.Add "FR_CableReportLevelNotFound", "ARES : Rapport de câbles — niveau des câbles introuvable : {0}"
    moTranslations.Add "FR_CableReportLevelsIgnored", "ARES : Rapport de câbles — niveau(x) de câbles ignoré(s) (introuvable) : {0}"
    moTranslations.Add "FR_CableReportCancelled", "ARES : Rapport de câbles — annulé"
    moTranslations.Add "FR_CableReportNoCables", "ARES : Rapport de câbles — aucun câble trouvé sur le niveau configuré"
    moTranslations.Add "FR_CableReportNoZones", "ARES : Rapport de câbles — aucune zone de tranchée trouvée ; les colonnes de nature de sol seront vides"
    moTranslations.Add "FR_CableReportPropertyInvalid", "ARES : Rapport de câbles — propriété {0} invalide, ignorée : {1}"
    moTranslations.Add "FR_CableReportComplete", "ARES : Rapport de câbles terminé — {0} câble(s), {1} colonne(s) de nature de sol, {2} incomplet(s), {3} zone(s) en tranchée commune"
    moTranslations.Add "FR_CableReportSheetName", "Rapport câbles"
    moTranslations.Add "FR_CableReportHeaderRepere", "Repère"
    moTranslations.Add "FR_CableReportHeaderNature", "Nature"
    moTranslations.Add "FR_CableReportHeaderLongueur", "Longueur"
    moTranslations.Add "FR_CableReportHeaderSharedWith", "Partagé avec"
    moTranslations.Add "FR_CableReportFooterCableTotal", "Total câbles"
    moTranslations.Add "FR_CableReportFooterTrenchTotal", "Tranchée à terrasser (dédoublonnée)"
    ' Formulaire d'options du Rapport de câbles
    moTranslations.Add "FR_CableReportGUIOptionsCaption", "Modifier les options du rapport de câbles :"
    moTranslations.Add "FR_CableReportGUIOptionsEditCableLevel_CommandCaption", "Modifier le niveau des câbles"
    moTranslations.Add "FR_CableReportGUIOptionsEditCableLevel_CommandTip", "Niveau contenant les traits (câbles HTAS) à recenser."
    moTranslations.Add "FR_CableReportGUIOptionsEditZoneLevel_CommandCaption", "Modifier le niveau des zones"
    moTranslations.Add "FR_CableReportGUIOptionsEditZoneLevel_CommandTip", "Niveau contenant les zones de tranchée (sortie de RunOutline). Vide = repli sur le niveau de sortie d'Outline."
    moTranslations.Add "FR_CableReportGUIOptionsRepereProperty_LabelCaption", "Propriété Repère :"
    moTranslations.Add "FR_CableReportGUIOptionsRepereProperty_LabelTip", "Propriété personnalisée lue sur chaque cellule d'extrémité pour le repère du câble. Doit être un ItemType de la DGNLib ARES."
    moTranslations.Add "FR_CableReportGUIOptionsNatureProperty_LabelCaption", "Propriété Nature :"
    moTranslations.Add "FR_CableReportGUIOptionsNatureProperty_LabelTip", "Propriété personnalisée lue sur le texte lié au câble pour sa nature/type. Doit être un ItemType de la DGNLib ARES."
    moTranslations.Add "FR_CableReportGUIOptionsLongueurProperty_LabelCaption", "Propriété Longueur :"
    moTranslations.Add "FR_CableReportGUIOptionsLongueurProperty_LabelTip", "Propriété personnalisée lue sur le texte lié au câble pour sa longueur totale. Doit être un ItemType de la DGNLib ARES."
    moTranslations.Add "FR_CableReportGUIOptionsZoneProperty_LabelCaption", "Propriété nature de sol :"
    moTranslations.Add "FR_CableReportGUIOptionsZoneProperty_LabelTip", "Propriété personnalisée lue sur chaque zone de tranchée ; répartie en une colonne par valeur distincte. Doit être un ItemType de la DGNLib ARES."
    moTranslations.Add "FR_CableReportGUIOptionsSearchRadius_LabelCaption", "Rayon de recherche :"
    moTranslations.Add "FR_CableReportGUIOptionsSearchRadius_LabelTip", "Rayon de recherche (unités maître) des cellules d'extrémité autour de chaque bout de câble. Doit être un nombre positif."
    moTranslations.Add "FR_CableReportGUIOptionsSearchRadiusError", "Le rayon de recherche doit être un nombre positif."
    moTranslations.Add "FR_CableReportGUIOptionsRound_LabelCaption", "Décimales :"
