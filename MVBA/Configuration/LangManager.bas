...
Sub InitializeTranslations()
    ...
    moTranslations.Add "EN_PropertyTaggingGUIOptionsEditRules_CommandTip", "Rules: level[:type]=prop|prop ; @CellName=prop attaches to the OTHER members of a graphic group containing a cell named CellName. Example: WALLS=Commune|Coupe_Type ; DOORS:Cell=Commune ; @ETI076=Commune"
    ...
    moTranslations.Add "EN_PropagationGUIOptionsMain_LabelTip", "When on, each @cell rule's label cell (defined in Property Tagging) writes its full text as the value of that rule's target property, onto the group members that already carry it. The trigger cells and target properties come from the @cell rules - there is no separate list here."
    moTranslations.Add "EN_PropagationGUIOptionsDetachEmpty_LabelCaption", "Remove emptied properties after calculation"
    moTranslations.Add "EN_PropagationGUIOptionsDetachEmpty_LabelTip", "When on, a property whose value is emptied by the calculation is detached (the tagger removes it) instead of kept empty; a rule that still mandates the property re-attaches it empty."
    moTranslations.Add "EN_PropagationValueRejected", "ARES: Property propagation - value rejected by the target property"
    moTranslations.Add "EN_PropagationNoTarget", "ARES: Property propagation - no group member carries the target property; enable Property Tagging and check the property is attached (DGNLib)"
    ...
    moTranslations.Add "FR_PropertyTaggingGUIOptionsEditRules_CommandTip", "Règles : niveau[:type]=prop|prop ; @NomCellule=prop attache aux AUTRES membres d'un groupe graphique contenant une cellule nommée NomCellule. Exemple : WALLS=Commune|Coupe_Type ; DOORS:Cell=Commune ; @ETI076=Commune"
    ...
    moTranslations.Add "FR_PropagationGUIOptionsMain_LabelTip", "Si activé, la cellule étiquette de chaque règle @cellule (définie dans l'étiquetage de propriété) écrit tout son texte comme valeur de la propriété cible de cette règle, sur les membres du groupe qui la portent déjà. Les cellules déclencheuses et les propriétés cibles proviennent des règles @cellule - il n'y a pas de liste séparée ici."
    moTranslations.Add "FR_PropagationGUIOptionsDetachEmpty_LabelCaption", "Supprimer les propriétés vidées après calcul"
    moTranslations.Add "FR_PropagationGUIOptionsDetachEmpty_LabelTip", "Si activé, une propriété dont la valeur est vidée par le calcul est détachée (le tagueur la retire) au lieu d'être conservée vide ; une règle qui impose encore la propriété la ré-attache vide."
    moTranslations.Add "FR_PropagationValueRejected", "ARES : Propagation de propriété - valeur refusée par la propriété cible"
    moTranslations.Add "FR_PropagationNoTarget", "ARES : Propagation de propriété - aucun membre du groupe ne porte la propriété cible ; activez l'étiquetage de propriété et vérifiez que la propriété est attachée (DGNLib)"
