...
Public Function SetVar(ByVal sKey As String, ByVal sValue As String) As Boolean
    On Error GoTo ErrorHandler
    ...
    If Not IsValidKey(sKey) Then Exit Function
    If sValue = vbNullString Then sValue = ""
    
    ' Set the configuration variable value
    Application.ActiveWorkspace.AddConfigurationVariable sKey, sValue, True
    SetVar = True
...
Public Function RemoveValue(ByVal sKey As String) As Boolean
    On Error GoTo ErrorHandler
...
Private Function IsValidKey(ByVal sKey As String) As Boolean
    On Error GoTo ErrorHandler
    
    IsValidKey = False
    
    ' Check for null, empty, or whitespace-only strings
    If sKey <> vbNullString And Len(Trim(sKey)) > 0 Then
        IsValidKey = True
    End If
